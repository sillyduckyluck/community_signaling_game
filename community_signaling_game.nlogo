...
[
  ...
  weight0A                              ;; association between signaling A and world-state 0
...
to find-path-lengths
  ...
  [
    ...
    [
      ...
      while [j < node-count]
      [
        ;; alternate path length through tth node
        set dummy ( (item t [distance-from-other-turtles] of turtle i) +
          (item j [distance-from-other-turtles] of turtle t))
...
to wire-them
  ...
  [
    ...
    while [ q < k / 2 ] [
      make-edge turtle n
      turtle ((n + 1 + q ) mod count turtles)
      set q q + 1 ]
    set n n + 1
  ]
end
